...
namespace DLS.Game
{
	public class Project
	{
		...
		public void AddNewRom(int addrBits, int dataBits)
		{
			if (description.CustomRomSizes == null) description.CustomRomSizes = new();
			KeyValuePair<int, int> pair = new(addrBits, dataBits);
			description.CustomRomSizes.Add(pair);
			ChipDescription romChip = BuiltinChipCreator.CreateCustomRom(addrBits, dataBits);
			chipLibrary.NotifyChipSaved(romChip);
			if (description.ChipCollections.Any(c => c.Name == "MEMORY"))
			{
				description.ChipCollections.First(c => c.Name == "MEMORY").Chips.Add(romChip.Name);
			}
		}
		...
		public void AddNewCustomDisplay(int widthBits, int heightBits, int bitsPerChannel)
		{
		    ...
		    ProjectDescription.CustomDisplaySettings settings = new() { WidthBits = widthBits, HeightBits = heightBits, BitsPerChannel = bitsPerChannel };
		    description.CustomDisplays.Add(settings);
		    ChipDescription displayChip = BuiltinChipCreator.CreateCustomDisplayRGB(settings);
		    chipLibrary.NotifyChipSaved(displayChip);
		    if (description.ChipCollections.Any(c => c.Name == "DISPLAY"))
		    {
		        description.ChipCollections.First(c => c.Name == "DISPLAY").Chips.Add(displayChip.Name);
		    }
		}

		public void AddNewCustomDisplayRGB(int widthBits, int heightBits, int bitsPerChannel)
		{
		    // Similar to AddNewCustomDisplay but ensures it's treated as a special RGB display
		    AddNewCustomDisplay(widthBits, heightBits, bitsPerChannel);
		}
    }
}