...
namespace DLS.Graphics
{
	public static class DevSceneDrawer
	{
		...
		static void DrawWires()
		{
			...
			orderedWires.AddRange(controller.ParallelWireTool.ParallelWiresToPlace);
		...
		{
			if (!Project.ActiveProject.description.Prefs_ShowAngleIndicator) return;

			WireInstance wire = null;
			int segmentIndex = -1;

			if (controller.IsCreatingWire)
			{
				wire = controller.WireToPlace;
				segmentIndex = wire.WirePointCount - 2;
			}
			else if (controller.isMovingWireEditPoint && controller.wireToEdit != null)
			{
				wire = controller.wireToEdit;
				segmentIndex = controller.wireEditPointIndex - 1;
				if (segmentIndex < 0) segmentIndex = 0;
			}

			if (wire != null && segmentIndex >= 0 && segmentIndex < wire.WirePointCount - 1)
			{
				Vector2 p1 = wire.GetWirePoint(segmentIndex);
				Vector2 p2 = wire.GetWirePoint(segmentIndex + 1);
				Vector2 dir = p2 - p1;
				...
				if (dir.sqrMagnitude > 0.001f)
				{
					...
					string angleText = Mathf.RoundToInt(angle).ToString() + "°";
					Vector2 textPos = InputHelper.MousePosWorld + new Vector2(0.2f, -0.2f);
					Draw.Text(FontBold, angleText, FontSizePinLabel, textPos, Anchor.TextCentreLeft, new Color(1, 1, 1, 0.5f));
				}
			}
		}
		...
		public static Bounds2D DrawDisplay(DisplayInstance display, Vector2 posParent, float parentScale, SubChipInstance rootChip, SimChip sim = null)
		{
			...
			else if (display.DisplayType == ChipType.DisplayRGB8x8)
			{
				bounds = DrawDisplay_RGB8x8(posWorld, scaleWorld, sim);
			}
			else if (display.DisplayType == ChipType.DisplayRGB64x64_XY)
			{
				bounds = DrawDisplay_RGB64x64_XY(posWorld, scaleWorld, sim);
			}
			else if (display.DisplayType == ChipType.DisplayRGB256x256_XY)
			{
				bounds = DrawDisplay_RGB256x256_XY(posWorld, scaleWorld, sim);
			}
			else if (display.DisplayType == ChipType.DisplayRGB64x64_XY_8bit)
			{
				bounds = DrawDisplay_RGB64x64_XY_8bit(posWorld, scaleWorld, sim);
			}
			else if (display.DisplayType == ChipType.DisplayRGB256x256_XY_8bit)
			{
				bounds = DrawDisplay_RGB256x256_XY_8bit(posWorld, scaleWorld, sim);
			}
			else if (display.DisplayType == ChipType.CustomDisplayRGB)
			{
				bounds = DrawDisplay_CustomRGB(posWorld, scaleWorld, sim, display.Desc.Name);
			}
		...
		public static Bounds2D DrawDisplay_RGB256x256_XY(Vector2 centre, float scale, SimChip simSource)
		{
			const int pixelsPerRow = 256;
			const float borderFrac = 0.95f;
			const float pixelSizeT = 0.925f;
			// Draw background
			Draw.Quad(centre, Vector2.one * scale, Color.black);
		...
		public static Bounds2D DrawDisplay_RGB64x64_XY_8bit(Vector2 centre, float scale, SimChip simSource)
		{
		...
		public static Bounds2D DrawDisplay_RGB256x256_XY_8bit(Vector2 centre, float scale, SimChip simSource)
		{
			const int pixelsPerRow = 256;
			const float borderFrac = 0.95f;
			const float pixelSizeT = 0.925f;
		...
		public static Bounds2D DrawDisplay_RGB64x64_XY(Vector2 centre, float scale, SimChip simSource)
		{
			const int pixelsPerRow = 64;
			const float borderFrac = 0.95f;
			const float pixelSizeT = 0.925f;
		...
		public static Bounds2D DrawDisplay_RGB8x8(Vector2 centre, float scale, SimChip simSource)
		{
			const int pixelsPerRow = 8;
			const float borderFrac = 0.95f;
			const float pixelSizeT = 0.925f;
			// Draw background
			Draw.Quad(centre, Vector2.one * scale, Color.black);
			...
			for (int y = 0; y < 8; y++)
			{
				for (int x = 0; x < 8; x++)
				{
					if (useSim)
					{
						int address = y * 8 + x;
						uint pixelState = simSource.InternalState[address];
						float red = Unpack2BitColChannel(pixelState);
						float green = Unpack2BitColChannel(pixelState >> 2);
						float blue = Unpack2BitColChannel(pixelState >> 4);
						col = new Color(red, green, blue);
					}
			...
			float Unpack2BitColChannel(uint raw)
			{
				return (raw & 0b11) / 3f;
			}
		}

		public static Bounds2D DrawDisplay_CustomRGB(Vector2 centre, float scale, SimChip simSource, string name)
		{
			// Parse "DISPLAY WxH (BBIT)"
			string[] parts = name.Split(' ');
			string[] dims = parts[1].Split('x');
			int width = int.Parse(dims[0]);
			int height = int.Parse(dims[1]);
			int bitsPerChannel = int.Parse(parts[2].Trim('(', ')', 'B', 'I', 'T'));
			...
			for (int y = 0; y < height; y++)
			{
				for (int x = 0; x < width; x++)
				{
					if (useSim)
					{
						...
						uint pixelState = simSource.InternalState[address];
						float red = UnpackCustomBitColChannel(pixelState, bitsPerChannel);
						float green = UnpackCustomBitColChannel(pixelState >> bitsPerChannel, bitsPerChannel);
						float blue = UnpackCustomBitColChannel(pixelState >> (bitsPerChannel * 2), bitsPerChannel);
						col = new Color(red, green, blue);
					}
			...
			float UnpackCustomBitColChannel(uint raw, int bits)
			{
				uint mask = (uint)((1 << bits) - 1);
				return (raw & mask) / (float)mask;
			}
		}
		...
		static bool ShouldHighlightWire(WireInstance wire)
		{
			...
			if (InteractionState.ElementUnderMousePrevFrame is WireInstance wireUnderMouse && wire == wireUnderMouse)
			{
				if (controller.IsCreatingWire)
				{
					return controller.CanCompleteWireConnection(controller.WireToPlace, wire, out PinInstance _);
				}
        ...
        static void DrawSingleBitPin(PinInstance pin)
		{
			...
			if (mouseOverPin && canInteract && controller.IsCreatingWire && !controller.CanCompleteWireConnection(controller.WireToPlace, pin))
			{
