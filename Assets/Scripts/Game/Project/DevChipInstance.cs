...
namespace DLS.Game
{
	public class DevChipInstance
	{
		...
		public PinInstance GetPinAtPos(Vector2 pos, float threshold = -1f)
		{
			float actualThreshold = threshold < 0 ? DrawSettings.PinRadius * 1.5f : threshold;
			foreach (IMoveable element in Elements)
			{
				if (element is SubChipInstance subchip)
				{
					...
					{
						if (Vector2.Distance(pin.GetWorldPos(), pos) < actualThreshold)
						{
							return pin;
						}
					}
				...
				{
					if (Vector2.Distance(devPin.Pin.GetWorldPos(), pos) < actualThreshold)
					{
						return devPin.Pin;
					}
				}
