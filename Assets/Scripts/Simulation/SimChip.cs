...
namespace DLS.Simulation
{
	public class SimChip
	{
		...
		public SimChip(ChipDescription desc, int id, uint[] internalState, SimChip[] subChips)
		{
			...
			else if (ChipType is ChipType.DisplayRGB8x8)
			{
				const int addressSize_6Bit = 64;
				// first 64 bits = display buffer, next 64 bits = back buffer, last bit = clock state (to allow edge-trigger behaviour)
				InternalState = new uint[addressSize_6Bit * 2 + 1];
			}
			else if (ChipType is ChipType.DisplayRGB64x64_XY)
			{
				const int addressSize_12Bit = 4096;
				// first 4096 bits = display buffer, next 4096 bits = back buffer, last bit = clock state (to allow edge-trigger behaviour)
				InternalState = new uint[addressSize_12Bit * 2 + 1];
			}
			else if (ChipType is ChipType.DisplayRGB256x256_XY)
			{
				const int addressSize_16Bit = 65536;
				// first 65536 bits = display buffer, next 65536 bits = back buffer, last bit = clock state (to allow edge-trigger behaviour)
				InternalState = new uint[addressSize_16Bit * 2 + 1];
			}
			else if (ChipType is ChipType.DisplayRGB64x64_XY_8bit)
			{
				const int addressSize_12Bit = 4096;
				InternalState = new uint[addressSize_12Bit * 2 + 1];
			}
			else if (ChipType is ChipType.DisplayRGB256x256_XY_8bit)
			{
				const int addressSize_16Bit = 65536;
				InternalState = new uint[addressSize_16Bit * 2 + 1];
			}
			...
			else if (ChipType is ChipType.CustomRom)
			{
				int addrBits = desc.InputPins[0].BitCount.BitCount;
				InternalState = new uint[(int)Math.Pow(2, addrBits)];
			}
			else if (ChipType is ChipType.CustomDisplayRGB)
			{
				...
				// first addressSpace bits = display buffer, next addressSpace bits = back buffer, 
				// then 2 slots for width and height, then 1 slot for clock state
				InternalState = new uint[addressSpace * 2 + 3];
