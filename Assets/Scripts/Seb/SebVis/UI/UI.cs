...
namespace Seb.Vis.UI
{
	public static class UI
	{
		...
		static Bounds2D scollContext_activeScrollArea;
		...
		{
			// Optimization for large lists (assuming fixed height based on first element)
			if (scollContext_activeScrollDrawCount > 200)
			{
				scollContext_activeScrollDrawElementFunc.Invoke(topLeft, width, 0, true);
				float itemHeight = PrevBounds.Height + scollContext_activeScrollDrawSpacing;

				if (isLayoutOnly)
				{
					float totalHeight = itemHeight * scollContext_activeScrollDrawCount - scollContext_activeScrollDrawSpacing;
					OverridePreviousBounds(Bounds2D.CreateFromTopLeftAndSize(topLeft, new Vector2(width, totalHeight)));
					return;
				}
				else
				{
					float scrollStateY = topLeft.y - scollContext_activeScrollArea.Top;
					int firstVisibleIndex = Mathf.FloorToInt(Mathf.Max(0, scrollStateY) / itemHeight);
					topLeft.y -= firstVisibleIndex * itemHeight;

					for (int i = firstVisibleIndex; i < scollContext_activeScrollDrawCount; i++)
					{
						scollContext_activeScrollDrawElementFunc.Invoke(topLeft, width, i, false);
						topLeft.y = PrevBounds.Bottom - scollContext_activeScrollDrawSpacing;

						if (topLeft.y < scollContext_activeScrollArea.Bottom - itemHeight) break;
					}
					return;
				}
			}
		...
		public static ScrollBarState DrawScrollView(UIHandle scrollID, Vector2 pos, Vector2 size, Anchor anchor, ScrollViewTheme theme, ScrollViewDrawContentFunc drawContentFunc)
		{
			...
			if (IsRendering)
			{
				...
				{
					scollContext_activeScrollArea = scrollArea;
		...
		public static ScrollBarState DrawScrollbar(Bounds2D scrollViewArea, Bounds2D scrollbarArea, float contentHeight, ScrollViewTheme theme, UIHandle id)
		{
			...
			if (IsRendering)
			{
				...
				float scrollBarHeight = Mathf.Max(theme.minScrollBarHeight, scrollbarArea.Height / contentOverflow);
