...
namespace DLS.Graphics
{
	public static class RomEditMenu
	{
		...
		static uint[] internalDataClone;
		static int selectedBank;
		static readonly UIHandle ID_BankInput = new("ROM_BankInput");
		static readonly Func<string, bool> integerInputValidator = ValidateBankInput;
		...
		public static void DrawMenu()
		{
			...
			using (UI.BeginBoundsScope(true))
			{
				...
				Vector2 nextUIElementPos = UI.PrevBounds.BottomLeft + Vector2.down * buttonSpacing;

				// Bank selector (for banked ROM)
				if (romChip.ChipType == ChipType.BankedRom)
				{
					InputFieldTheme bankInputTheme = MenuHelper.Theme.ChipNameInputField;
					bankInputTheme.fontSize = MenuHelper.Theme.FontSizeRegular;
					InputFieldState bankInputState = UI.InputField(ID_BankInput, bankInputTheme, nextUIElementPos, new Vector2(sidePanelSize.x, DrawSettings.SelectorWheelHeight), "0", Anchor.TopLeft, 1, integerInputValidator);

					if (int.TryParse(bankInputState.text, out int bankNew))
					{
						bankNew = Mathf.Clamp(bankNew, 0, 255);
						if (bankNew != selectedBank)
						{
							StoreCurrentBankInClone();
							selectedBank = bankNew;
							UpdateUIFromClone();
						}
					}
					nextUIElementPos = UI.PrevBounds.BottomLeft + Vector2.down * buttonSpacing;
				}


				int copyPasteButtonIndex = MenuHelper.DrawButtonPair("COPY ALL", "PASTE ALL", nextUIElementPos, sidePanelSize.x, false);
				Vector2 buttonTopleft = UI.PrevBounds.BottomLeft + Vector2.down * buttonSpacing;
		...
		{
			StoreCurrentBankInClone();
			Array.Copy(internalDataClone, romChip.InternalData, internalDataClone.Length);
		...
		public static void OnMenuOpened()
		{
			...
			internalDataClone = (uint[])romChip.InternalData.Clone();

			selectedBank = 0;
			UI.GetInputFieldState(ID_BankInput).SetText("0");

			ActiveRomDataBitCount = 16;
			if (romChip.ChipType == ChipType.CustomRom)
			{
				ActiveRomDataBitCount = romChip.Description.OutputPins[0].BitCount.BitCount;
			}
			else if (romChip.ChipType == ChipType.Rom_256x16 || romChip.ChipType == ChipType.EEPROM_256x16 || romChip.ChipType == ChipType.BankedRom)
			{
				ActiveRomDataBitCount = 16;
			}

			RowCount = internalDataClone.Length;
			if (romChip.ChipType == ChipType.BankedRom) RowCount = 256;
		...
		static void UpdateUIFromClone()
		{
			int lineNumberPadLength = RowCount.ToString().Length;
			int offset = selectedBank * RowCount;

			for (int i = 0; i < RowCount; i++)
			{
				if (IDS_inputRow[i].stringID == null) IDS_inputRow[i] = new UIHandle("ROM_rowInputField", i);
				InputFieldState state = UI.GetInputFieldState(IDS_inputRow[i]);

				string displayString = UIntToDisplayString(internalDataClone[i + offset], dataDisplayMode, ActiveRomDataBitCount);
		...
		static void StoreCurrentBankInClone()
		{
			int offset = selectedBank * RowCount;
			for (int i = 0; i < RowCount; i++)
			{
				...
				internalDataClone[i + offset] = newValue;
			}
		}
		...
		static bool ValidateBankInput(string s)
		{
