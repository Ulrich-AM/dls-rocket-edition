...
namespace DLS.Description
{
    public struct ProjectDescription
    {
		...
		public int Prefs_ParallelWires;
		public float Prefs_ParallelWireSnapThreshold;
		...
		[Serializable]
		public struct CustomDisplaySettings
		{
			public int WidthBits;
			public int HeightBits;
			public int BitsPerChannel;

			public string Name => $"DISPLAY {Math.Pow(2, WidthBits)}x{Math.Pow(2, HeightBits)} ({BitsPerChannel}BIT)";
		}
		...
		{
			List<String> result = new List<String>();
			if (CustomRomSizes == null) return result;
			foreach (var pair in CustomRomSizes)
			{
				result.Add($"ROM {(long)Math.Pow(2, pair.Key)}x{pair.Value}");
			}
			return result;
		}

		public List<String> CustomDisplayNames()
		{
			List<String> result = new List<String>();
			if (CustomDisplays == null) return result;
		...
		public List<String> AllSpecialNames()
		{
			...
			result.AddRange(RomNames());
			result.AddRange(CustomDisplayNames());
		...
		public void RemoveSpecial(string name) {
			...
			else if (RomNames().Contains(name))
			{
				RemoveRom(name);
			}
			else if (CustomDisplayNames().Contains(name))
			{
				RemoveCustomDisplay(name);
			}
		}
		...
		void RemoveRom(string name)
		{
			int index = RomNames().IndexOf(name);
			CustomRomSizes.RemoveAt(index);
		}

		void RemoveCustomDisplay(string name)
		{
			int index = CustomDisplayNames().IndexOf(name);
			CustomDisplays.RemoveAt(index);
		}
