...
namespace DLS.Game
{
	public class ChipInteractionController
	{
		...
		public ParallelWireTool ParallelWireTool = new();
		...
		public bool CanCompleteWireConnection(WireInstance wireBeingPlaced, WireInstance wireToConnectTo, out PinInstance endPin)
		{
			// If we're joining this wire to an existing wire, choose the appropriate source/target pin from that wire to connect to
			endPin = wireBeingPlaced.FirstPin.IsSourcePin ? wireToConnectTo.TargetPin_BusCorrected : wireToConnectTo.SourcePin;
			return CanCompleteWireConnection(wireBeingPlaced, endPin, wireToConnectTo);
		}

		public bool CanCompleteWireConnection(WireInstance wireBeingPlaced, PinInstance endPin, WireInstance wireToConnectTo = null)
		{
			if (wireBeingPlaced == null) return false;

			PinInstance startPin = wireBeingPlaced.FirstPin;
			bool connectingFromWire = wireBeingPlaced.FirstConnectionInfo.IsConnectedAtWire;
			bool connectingToWire = wireToConnectTo != null;
			WireInstance wireConnection = wireToConnectTo ?? wireBeingPlaced.FirstConnectionInfo.connectedWire;
		...
		void HandleKeyboardInput()
		{
			...
			if (KeyboardShortcuts.DeleteShortcutTriggered())
			{
				if (IsCreatingWire)
				{
					...
					{
						ParallelWireTool.RemoveLastPoint();
					}
				}
				else
				{
		...
		void HandleMouseInput()
		{
			...
			// Parallel wire spacing adjustment
			if (IsCreatingWire && ParallelWireTool.ParallelWiresToPlace.Count > 0 && project.description.Prefs_ParallelWires != 0 && InputHelper.CtrlIsHeld)
			{
				ParallelWireTool.ParallelWiresSpacingFactor += InputHelper.MouseScrollDelta.y * 0.1f;
				ParallelWireTool.ParallelWiresSpacingFactor = Mathf.Max(0, ParallelWireTool.ParallelWiresSpacingFactor);
			}
		}
		...
		void HandleLeftMouseDown()
		{
			...
			if (IsPlacingElementOrCreatingWire && !InputHelper.LockMode)
			{
				...
				if (IsCreatingWire) //
				{
					...
					{
						Vector2 mousePos = InputHelper.MousePosWorld;
						WireToPlace.AddWirePoint(mousePos);

						if (ParallelWireTool.ParallelWiresToPlace.Count > 0)
						{
							ParallelWireTool.AddPoint(mousePos, WireToPlace.GetWirePoint(0));
						}
					}
				}
		...
		void StartPlacingWire(WireInstance.ConnectionInfo connectionInfo)
		{
			...
			ParallelWireTool.Clear();
			ParallelWireTool.ParallelWiresInitialDir = connectionInfo.pin.FacingDir;

			if (project.description.Prefs_ParallelWires != 0 && SelectedElements.Count > 1)
			{
				if (connectionInfo.pin.parent is DevPinInstance clickedDevPin && clickedDevPin.IsSelected)
				{
					foreach (var element in SelectedElements)
					{
						if (element is DevPinInstance otherDevPin && otherDevPin != clickedDevPin)
						{
							if (otherDevPin.IsInputPin == clickedDevPin.IsInputPin && otherDevPin.BitCount.BitCount == clickedDevPin.BitCount.BitCount)
							{
								int sOrder = project.ViewedChip.Wires.Count > 0 ? project.ViewedChip.Wires[^1].spawnOrder + 1 + ParallelWireTool.ParallelWiresToPlace.Count : ParallelWireTool.ParallelWiresToPlace.Count;
								WireInstance.ConnectionInfo otherInfo = new() { pin = otherDevPin.Pin };
								ParallelWireTool.ParallelWiresToPlace.Add(new WireInstance(otherInfo, sOrder));
							}
						}
					}
				}
			...
			ClearSelection();
			int spawnOrder = project.ViewedChip.Wires.Count > 0 ? project.ViewedChip.Wires[^1].spawnOrder + 1 + ParallelWireTool.ParallelWiresToPlace.Count : ParallelWireTool.ParallelWiresToPlace.Count;
		...
		void UpdatePositionsToMouse()
		{
			...
			if (IsCreatingWire)
			{
				...
				if (ParallelWireTool.ParallelWiresToPlace.Count > 0)
				{
					ParallelWireTool.UpdatePositions(mousePos, WireToPlace.GetWirePoint(0));
				}
			}
		...
		bool TryFinishPlacingWire()
		{
			...
			{
				if (CanCompleteWireConnection(WireToPlace, pin))
				{
					WireInstance.ConnectionInfo info = new() { pin = pin };
					CompleteConnection(WireToPlace, info);

					foreach (var parallelWire in ParallelWireTool.ParallelWiresToPlace)
					{
						Vector2 endPos = parallelWire.GetWirePoint(parallelWire.WirePointCount - 1);
						PinInstance targetPin = project.ViewedChip.GetPinAtPos(endPos, project.description.Prefs_ParallelWireSnapThreshold);
						if (targetPin != null && CanCompleteWireConnection(parallelWire, targetPin))
						{
							WireInstance.ConnectionInfo pInfo = new() { pin = targetPin };
							CompleteConnection(parallelWire, pInfo);
						}
					}
					ParallelWireTool.Clear();
			...
			{
				if (CanCompleteWireConnection(WireToPlace, connectionWire, out PinInstance endPin))
				{
			...
			void CompleteConnection(WireInstance wire, WireInstance.ConnectionInfo info)
			{
				wire.FinishPlacingWire(info);
				ActiveDevChip.AddWire(wire, false);
		...
		IMoveable CreateElementFromChipDescription(ChipDescription chipDescription)
		{
			...
			{
				SubChipDescription subChipDesc = DescriptionCreator.CreateBuiltinSubChipDescriptionForPlacement(chipDescription, instanceID, Vector2.zero);
