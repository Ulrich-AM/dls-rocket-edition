	...
	public static class BuiltinChipCreator
	{
		...
		public static ChipDescription[] CreateAllBuiltinChipDescriptions(ProjectDescription description)
		{
			...
			return new[]
			{
				...
				CreateDisplayDot(),
				CreateDisplayLED(),
				CreateDisplayRGB64x64_XY(),
				CreateDisplayRGB256x256_XY(),
				CreateDisplayRGB64x64_XY_8bit(),
				CreateDisplayRGB256x256_XY_8bit(),
				// ---- Audio ----
				CreateBuzzer(),
				// ---- Clock ----
				CreateSPSChip(),
		...
		{
			if (displaySettings == null) return Array.Empty<ChipDescription>();
			ChipDescription[] descriptions = new ChipDescription[displaySettings.Count];
			for (int i = 0; i < displaySettings.Count; i++)
			{
				descriptions[i] = CreateCustomDisplayRGB(displaySettings[i]);
			}
			return descriptions;
		}

		public static ChipDescription CreateCustomDisplayRGB(ProjectDescription.CustomDisplaySettings settings)
		{
			int addrBits = settings.WidthBits + settings.HeightBits;
			int colorBits = settings.BitsPerChannel * 3;

			float height = GridSize * (Mathf.Max((int)Math.Pow(2, settings.HeightBits), (int)Math.Pow(2, settings.WidthBits)) + 6);
			float width = height;
			...
			PinDescription[] inputPins =
			{
				...
				CreatePinDescription("COLOUR", 1, (ushort)colorBits),
			...
			{
				CreatePinDescription("COL OUT", 6, (ushort)colorBits)
			};
			...
			return CreateBuiltinChipDescription(ChipType.CustomDisplayRGB, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, name: settings.Name, canBeCached: false);
		}

		static ChipDescription[] CreateCustomRoms(List<KeyValuePair<int, int>> romSizes)
		{
			if (romSizes == null) return Array.Empty<ChipDescription>();
			ChipDescription[] descriptions = new ChipDescription[romSizes.Count];
			for (int i = 0; i < romSizes.Count; i++)
			{
				descriptions[i] = CreateCustomRom(romSizes[i].Key, romSizes[i].Value);
			}
			return descriptions;
		}

		public static ChipDescription CreateCustomRom(int addrBits, int dataBits)
		{
			PinDescription[] inputPins =
			{
				CreatePinDescription("ADDRESS", 0, (ushort)addrBits)
			...
			Vector2 minChipSize = SubChipInstance.CalculateMinChipSize(inputPins, outputPins, name);
			Vector2 size = new(Mathf.Max(GridSize * 12, minChipSize.x), minChipSize.y);

			return CreateBuiltinChipDescription(ChipType.CustomRom, size, col, inputPins, outputPins, name: name);
		}
		...
		static ChipDescription CreateBankedROM()
		{
			PinDescription[] inputPins =
			{
				CreatePinDescription("SELECTOR", 0, PinBitCount.Bit8),
				CreatePinDescription("ADDRESS", 1, PinBitCount.Bit8)
			};
			PinDescription[] outputPins =
			{
			...
			Color col = GetColor(new(0.25f, 0.35f, 0.5f));
			Vector2 size = new(GridSize * 14, SubChipInstance.MinChipHeightForPins(inputPins, outputPins));

			return CreateBuiltinChipDescription(ChipType.BankedRom, size, col, inputPins, outputPins);
		}
		...
		static ChipDescription CreateDisplayRGB8x8()
		{
			float height = GridSize * 14;
			float width = height;
			float displayWidth = height - GridSize * 2;
			...
			PinDescription[] inputPins =
			{
				CreatePinDescription("ADDRESS", 0, 6),
				CreatePinDescription("COLOUR", 1, 6),
				CreatePinDescription("RESET", 2),
				CreatePinDescription("WRITE", 3),
				CreatePinDescription("REFRESH", 4),
				CreatePinDescription("CLOCK", 5)
			...
			PinDescription[] outputPins =
			{
				CreatePinDescription("COL OUT", 6, 6)
			};
			...
			return CreateBuiltinChipDescription(ChipType.DisplayRGB8x8, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, canBeCached: false);
		}
		...
		static ChipDescription CreateDisplayRGB64x64_XY()
		{
			float height = GridSize * 23;
			float width = height;
			float displayWidth = height - GridSize * 2;
			...
			PinDescription[] inputPins =
			{
				CreatePinDescription("X", 0, 6),
				CreatePinDescription("Y", 1, 6),
				CreatePinDescription("RED", 2, 4),
				CreatePinDescription("GREEN", 3, 4),
				CreatePinDescription("BLUE", 4, 4),
				CreatePinDescription("RESET", 5),
				CreatePinDescription("WRITE", 6),
				CreatePinDescription("REFRESH", 7),
				CreatePinDescription("CLOCK", 8)
			};

			PinDescription[] outputPins =
			{
				CreatePinDescription("R OUT", 9, 4),
				CreatePinDescription("G OUT", 10, 4),
				CreatePinDescription("B OUT", 11, 4)
			};
			...
			return CreateBuiltinChipDescription(ChipType.DisplayRGB64x64_XY, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, canBeCached: false);
		}

		static ChipDescription CreateDisplayRGB256x256_XY()
		{
			float height = GridSize * 25;
			float width = height;
			float displayWidth = height - GridSize * 2;
			...
			PinDescription[] inputPins =
			{
				CreatePinDescription("X", 0, 8),
				CreatePinDescription("Y", 1, 8),
				CreatePinDescription("RED", 2, 4),
				CreatePinDescription("GREEN", 3, 4),
				CreatePinDescription("BLUE", 4, 4),
				CreatePinDescription("RESET", 5),
			...
			return CreateBuiltinChipDescription(ChipType.DisplayRGB256x256_XY, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, canBeCached: false);
		}

		static ChipDescription CreateDisplayRGB64x64_XY_8bit()
		{
			float height = GridSize * 26;
			float width = height;
			float displayWidth = height - GridSize * 2;
			...
			PinDescription[] inputPins =
			{
				CreatePinDescription("X", 0, 6),
				CreatePinDescription("Y", 1, 6),
				CreatePinDescription("RED", 2, 8),
				CreatePinDescription("GREEN", 3, 8),
				CreatePinDescription("BLUE", 4, 8),
				CreatePinDescription("RESET", 5),
				CreatePinDescription("WRITE", 6),
				CreatePinDescription("REFRESH", 7),
				CreatePinDescription("CLOCK", 8)
			...
			PinDescription[] outputPins =
			{
				CreatePinDescription("R OUT", 9, 8),
				CreatePinDescription("G OUT", 10, 8),
				CreatePinDescription("B OUT", 11, 8)
			};
			...
			return CreateBuiltinChipDescription(ChipType.DisplayRGB64x64_XY_8bit, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, canBeCached: false);
		}

		static ChipDescription CreateDisplayRGB256x256_XY_8bit()
		{
			float height = GridSize * 28;
			float width = height;
			float displayWidth = height - GridSize * 2;
			...
			return CreateBuiltinChipDescription(ChipType.DisplayRGB256x256_XY_8bit, size, col, inputPins, outputPins, displays, NameDisplayLocation.Hidden, canBeCached: false);
		}
		...
		{
			if (string.IsNullOrEmpty(name) && !ChipTypeHelper.IsDevPin(type) && !ChipTypeHelper.IsMergeSplitChip(type) && !ChipTypeHelper.IsBusType(type)){name = ChipTypeHelper.GetName(type); }
